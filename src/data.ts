...
export const JAKARTA_LON_MAX = 107.0;
...
export function toEPSG3857(lat: number, lon: number): [number, number] {
  ...
  const y =
    Math.log(Math.tan(((90.0 + latClamped) * Math.PI) / 360.0)) * R_EARTH;
  return [x, y];
}
...
export const BASE_STATIONS: { name: string; lat: number; lon: number }[] = [
  { name: "Kemayoran BMKG", lat: -6.155, lon: 106.84 },
  ...
  { name: "Kebayoran Baru Grid", lat: -6.23, lon: 106.804 },
...
export const BASE_SENSORS: {
  ...
  { name: "Pintu Air Karet", lat: -6.2, lon: 106.818, baseLvl: 160, flash: 45 },
  {
    name: "Pintu Air Pasar Ikan",
    lat: -6.125,
    lon: 106.808,
    baseLvl: 140,
    flash: 35,
  ...
  {
    name: "Cipinang Hulu Monitoring",
    lat: -6.292,
    lon: 106.878,
    baseLvl: 170,
    flash: 50,
  },
  { name: "Pos Sunter Hulu", lat: -6.28, lon: 106.91, baseLvl: 190, flash: 48 },
  { name: "Pos Angke Hulu", lat: -6.31, lon: 106.715, baseLvl: 210, flash: 52 },
  {
  ...
  {
    name: "Ciliwung Katulampa (Remote)",
    lat: -6.4,
    lon: 106.85,
    baseLvl: 260,
    flash: 80,
...
export const SIMULATION_PRESETS: SimulationPreset[] = [
  {
    ...
    description:
      "Typical dry season baseline. Sparse localized showers, normal river channels, and all floodgates operating below warning level (Siaga 4).",
    rainMultiplier: 0.15,
    coastalTideCm: 80,
  ...
  },
  {
    ...
    description:
      "Intense, widespread rainfall across the Ciliwung river catchment basin. Severe overflow at Manggarai and Bukit Duri (Siaga 1 warnings).",
    rainMultiplier: 1.35,
    coastalTideCm: 120,
  ...
  },
  {
    ...
    description:
      "High astronomically-driven spring tides flooding the Northern marshes of Pluit, Penjaringan, and Marina, aggravated by local downpours.",
    rainMultiplier: 0.85,
    coastalTideCm: 220,
  ...
  },
  {
    ...
    description:
      "Sudden, extremely heavy localized afternoon cloudburst in Southern Jakarta hills. Rapid flash rises in Pos Pesanggrahan and Pos Angke.",
    rainMultiplier: 1.5,
    coastalTideCm: 90,
...
export function generateJakartaScaffolding(numRts: number = 30000): {
  ...
  for (let i = 0; i < 120; i++) {
    ...
    const lat =
      i < BASE_SENSORS.length ? base.lat : base.lat + rand.range(-0.04, 0.04);
    const lon =
      i < BASE_SENSORS.length ? base.lon : base.lon + rand.range(-0.04, 0.04);
    ...
    sensors.push({
      sensor_id: sId,
      name:
        i < BASE_SENSORS.length
          ? base.name
          : `${base.name.replace("Monitoring", "").replace("Pos", "").trim()} Aux ${i - BASE_SENSORS.length + 1}`,
      lat,
      lon,
  ...
  for (let i = 0; i < numRts; i++) {
    ...
    const elevationTrend =
      (JAKARTA_LAT_MAX - lat) / (JAKARTA_LAT_MAX - JAKARTA_LAT_MIN); // 0 (North) to 1 (South)
    ...
    rts.push({
      ...
      population_density: 5000 + rand.next() * 40000,
