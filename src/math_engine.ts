 ...
 */
export function calculateGevCdf(
  x: number,
  mu: number,
  sigma: number,
  xi: number,
...
export function runHydrologicalPipeline(
  ...
  computedSensors.forEach((s) =>
    sensorProbMap.set(s.sensor_id, s.exceedance_prob),
  );
  ...
  for (let i = 0; i < numRts; i++) {
    ...
    rt.risk_priority_score =
      w1 * rt.evt_exceedance_prob + w2 * rainNorm + w3 * elevationRisk;
  }
